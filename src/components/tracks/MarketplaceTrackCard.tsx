...
import { ShoppingCart, MusicNote, Eye } from '@phosphor-icons/react'
...
export function MarketplaceTrackCard({ track, onPurchase }: MarketplaceTrackCardProps) {
  ...
  // Mock artist name since it's not in Track type - derive from artistId
  const artistName = track.artistId === 'user_001' ? 'Demo Artist' : 'Independent Artist'

  return (
    <Card className={cn(
      'overflow-hidden border-border/60 transition-all duration-200 group',
      'hover:border-accent/60 hover:shadow-lg hover:shadow-accent/5 hover:-translate-y-1 hover:scale-[1.02]',
      ...
      <CardContent className="p-0">
        <div className="aspect-square relative bg-muted overflow-hidden">
          ...
          >
            <span className="flex items-center gap-2 text-white font-medium text-sm bg-black/50 px-4 py-2 rounded-full backdrop-blur-sm">
              <Eye size={18} />
              View Details
            </span>
          </Link>
